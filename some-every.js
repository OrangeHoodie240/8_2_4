...
function hasOddNumber(arr) {
    return arr.some(b => b % 2 === 1); 
}

/*
Write a function called hasAZero which accepts a number and returns
 true if that number contains at least one zero. Otherwise, the function should return false
...
function hasAZero(num) {
    return Array.from(String(num)).some(b => b === '0'); 
}
...
function hasCertainKey(arr, key) {
    return arr.every(b => b.hasOwnProperty(key));
}

/*
Write a function called hasCertainValue which accepts an array of 
objects and a key, and a value, and returns true if every single 
...
function hasCertainValue(arr, key, searchValue) {
    return arr.every(b => b[key] === searchValue); 
}
